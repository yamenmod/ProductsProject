...
const updateSetting = async (req, res) => {
  try {
    ...
    // Validate max_products_per_cart if updating it
    if (key === "max_products_per_cart") {
      const numValue = parseInt(value, 10);
      if (isNaN(numValue) || numValue < 1) {
        return res
          .status(400)
          .json({ error: "Max products per cart must be a positive integer" });
      }

      if (numValue > 1000) {
        return res
          .status(400)
          .json({ error: "Max products per cart cannot exceed 1000" });
      }
    ...
    // Validate max_quantity_per_product if updating it
    if (key === "max_quantity_per_product") {
      const numValue = parseInt(value, 10);
      if (isNaN(numValue) || numValue < 1) {
        return res
          .status(400)
          .json({ error: "Max quantity per product must be a positive integer" });
      }

      if (numValue > 1000) {
        return res
          .status(400)
          .json({ error: "Max quantity per product cannot exceed 1000" });
      }
